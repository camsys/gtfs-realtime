      ...
      def process
        ...
        if @gtfs_realtime_configuration.trip_updates_feed.present?
         ...
         if GTFS::Realtime::Feed.from_partition(@gtfs_realtime_configuration.id, Date.today.at_beginning_of_week).where(class_name: 'TripUpdate', feed_status_type_id: GTFS::Realtime::FeedStatusType.find_by(name: 'Running').id).count == 0
           GTFS::Realtime::Feed.from_partition(@gtfs_realtime_configuration.id, Date.today.at_beginning_of_week).order(:feed_timestamp).where(class_name: 'TripUpdate', feed_status_type_id: nil).limit(@gtfs_realtime_configuration.interval_seconds/10).each do |trip_updates_feed|
             start_time = Time.now
