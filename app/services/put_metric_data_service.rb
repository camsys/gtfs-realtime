class PutMetricDataService
  ...
  def initialize
    @cw = Fog::AWS::CloudWatch.new({:aws_secret_access_key => ENV['CLOUDWATCH_AWS_SECRET_KEY'],
                                    :aws_access_key_id => ENV['CLOUDWATCH_AWS_ACCESS_KEY']
                                   }) unless ENV['CLOUDWATCH_AWS_SECRET_KEY'].blank? && ENV['CLOUDWATCH_AWS_ACCESS_KEY'].blank?
    @env = ENV['RAILS_ENV']
